...
%                             Beeper(1000,.9); % high beep if you made the time limit
        ...
        for x = 1:length(SOT_data)
            dlmwrite(strcat(pathdata,'\',subjNum,'_',num2str(blockNum),'_graspExp.txt'),...
                [trials,SOT_data(x),xy1_data1(x),xy1_data2(x),xy2_data1(x),xy2_data2(x),currXYZ1_data1(x),...
        ...
        %%%FIGURE THIS OUT: OUTPUT SPEED AND WIDTH DATA TO EASIER-TO-READ FILE
        dlmwrite(strcat(pathdata,'\',subjNum,'_',num2str(blockNum),'_graspExp_info.txt'),...
            [trials, r1, r2, reachedTo, ISI_onset_time(trials)-S1_onset_time(trials), S2_onset_time(trials)-S1_onset_time(trials),...
            timeElapsed(trials), Keyresponse(trials), angleGraspInitial(trials), acc(trials), acc_grasp(trials),markerWait(trials)...
            ],'-append', 'roffset',...
            [],'delimiter', '\t');
