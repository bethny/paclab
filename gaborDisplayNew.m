...
atLab = 0;
directories = {'~/code','~/Desktop/Bethany/paclab'};
...
contrast = 1;
gsize = 8;      %size of the grating
vsize = 2.5;      %size of the visible grating (edge size)
...
%     gabortex = CreateProceduralGabor(win, si, si, 1);
    ...
    m = grey*contrast*makegabor(gaborDimPix,freq,sigma,0,VgaborDimPix,1); %make gabor
    ...
    if crowding        
        dstRects(:,2) = CenterRectOnPoint(texrect, refPtX + radialDist, refPtY)'; % rightmost
        ...
        dstRects(:,5) = CenterRectOnPoint(texrect, refPtX - radialDist, refPtY)'; % leftmost
