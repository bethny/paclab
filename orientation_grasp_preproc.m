...
for s = 1:nSubj
    ...
    blockLength = 64;
    nBlocks = 4;
    blockLength = blockLength * nBlocks;
    % mark as "1" if you want to upsample data
    upSample = 0;
    samplingRate = 200; % should this be 160? 
...
%     for currentSub = curSubj
        ...
        if saveFiles && ~reUseThresholds
            ...
            outputFilename = [subFolder,filesep,'RedoTrials_',curSubj];
            ...
            % changed) and open it
            outputFilename2 = [subFolder,filesep,'ThresholdAdjustments_',curSubj];
        ...
        for allFiles = curSubj % Can be changed to pre-process multiple subjects at once.  Should
...
%                 if upSample % don't worry about this for now
...
%                 end
                ...
                totalArmChange = sqrt(xArmChange.^2 + yArmChange.^2 + zArmChange.^2); % Calculate the distance (currently 2D)
                ...
                while j <= (length(trialStartIndex))% Giant loop to deal with data from each trial
                    ...
                    if ~noMovement(overallCounter)  && ~droppedTrials(overallCounter) %Only bother calculating curvature
                        ...
                        if drawImages &&  (startMiddle == 0 || ( j>=trialEnd ))
                            ...
                            tempTitle = (strcat('Subject #', num2str(allFiles),'Trial #',num2str(j), '__', num2str(i), '___',...
                                'MaxCurvature', num2str(maxDeviation(overallCounter) *signOfCurvature(overallCounter)), '___',...
                                'accuracy', num2str(acc(overallCounter))));
                            title(tempTitle);
                            hold off;
